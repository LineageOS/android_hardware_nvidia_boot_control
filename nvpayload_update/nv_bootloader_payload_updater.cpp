...
BLStatus NvPayloadUpdate::OTAUpdater(const char* ota_path) {
    ...
    if (strncmp(header->magic, "NVIDIA__BLOB__V2", UPDATE_MAGIC_SIZE) == 0)
        entry_len += IMG_SPEC_INFO_LENGTH_V2;
    else if (strncmp(header->magic, "NVIDIA__BLOB__V3", UPDATE_MAGIC_SIZE) == 0)
...
void NvPayloadUpdate::ParseEntryTable(char* buffer, std::vector<Entry>& entry_table,
    ...
    if (strncmp(header->magic, "NVIDIA__BLOB__V3", UPDATE_MAGIC_SIZE) == 0)
