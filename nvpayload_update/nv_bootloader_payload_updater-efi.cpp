...
std::string NvPayloadUpdate::GetDeviceTNSpec() {
    ...
    char *tnspec = NULL;
    uint32_t attributes;
    if (efi_get_variable(EFI_NVIDIA_PUBLIC_VARIABLE_GUID, "TegraPlatformCompatSpec",
...
BLStatus NvPayloadUpdate::FMPSetup() {
    ...
    std::ifstream infile(CAPSULE_PATH, std::ios::in | std::ios::binary);
    if (!infile.is_open())
        return kBlobOpenFailed;
    ...
    std::ofstream outdev(PARTITION_PATH "/" STAGING_PART, std::ios::out | std::ios::binary);
    if (!outdev.is_open())
        return kInternalError;

    outdev.rdbuf()->pubsetbuf(nullptr, 0);

    outdev.write(buffer.data(), buffer.size());
    if (!outdev.good()) {
        std::cout << "Failed to write " << CAPSULE_PATH << " to " << STAGING_PART << std::endl;
        return kInternalError;
    }
