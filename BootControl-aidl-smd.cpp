...
#include "BootControl-aidl-smd.h"
...
ScopedAStatus BootControl::getActiveBootSlot(int32_t* _aidl_return) {
    ...
    for (uint8_t i = 0 ; i < smd_partition.num_slots; i++) {
        if (smd_partition.slot_info[i].priority > high_pri) {
            high_pri = smd_partition.slot_info[i].priority;
            *_aidl_return = i;
        }
    }
...
ScopedAStatus BootControl::getCurrentSlot(int32_t* _aidl_return) {
    ...
    return ScopedAStatus::fromServiceSpecificErrorWithMessage(COMMAND_FAILED,
                                                              "Failed to find current slot");
}

bool BootControl::setBootSuccessful(int32_t slot) {
    smd_partition_t smd_partition;
...
ScopedAStatus BootControl::markBootSuccessful() {
    int32_t slot = 0;
    if (!getCurrentSlot(&slot).isOk() || slot < 0 || slot >= MAX_SLOTS)
        return ScopedAStatus::fromServiceSpecificErrorWithMessage(
                INVALID_SLOT, (std::string("Invalid slot ") + std::to_string(slot)).c_str());
...
ScopedAStatus BootControl::setActiveBootSlot(int32_t slot) {
    ...
    if (!getCurrentSlot(&slot_s).isOk() || slot < 0 || slot >= MAX_SLOTS)
        return ScopedAStatus::fromServiceSpecificErrorWithMessage(
                INVALID_SLOT, (std::string("Invalid source slot ") + std::to_string(slot)).c_str());
    ...
    if (slot >= smd_partition.num_slots)
        return ScopedAStatus::fromServiceSpecificErrorWithMessage(COMMAND_FAILED,
                                                                  "Requested slot is larger than available slots");
...
ScopedAStatus BootControl::isSlotBootable(int32_t slot, bool* _aidl_return) {
    smd_partition_t smd_partition;
    ...
    *_aidl_return = (smd_partition.slot_info[slot].priority != 0);

    return ScopedAStatus::ok();
}

ScopedAStatus BootControl::isSlotMarkedSuccessful(int32_t slot, bool* _aidl_return) {
    smd_partition_t smd_partition;
    ...
    *_aidl_return = smd_partition.slot_info[slot].boot_successful;

    return ScopedAStatus::ok();
}

ScopedAStatus BootControl::getSuffix(int32_t slot, std::string* _aidl_return) {
    smd_partition_t smd_partition;

    if (!readSlotMetadata(&smd_partition)) {
        // Old HIDL hal returns empty string for read failure. We should maintain this behavior in
    ...
    *_aidl_return = std::string(smd_partition.slot_info[slot].suffix, 2);
